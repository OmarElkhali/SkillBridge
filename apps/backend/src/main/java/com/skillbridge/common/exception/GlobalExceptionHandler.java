...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiErrorResponse> handleConstraintViolation(ConstraintViolationException ex, HttpServletRequest request) {
        return build(HttpStatus.BAD_REQUEST, "Validation failed.", request.getRequestURI(),
                ex.getConstraintViolations().stream().map(Object::toString).toList());
    }
    ...
    public ResponseEntity<ApiErrorResponse> handleGeneric(Exception ex, HttpServletRequest request) {
        ...
        return build(HttpStatus.INTERNAL_SERVER_ERROR, "An unexpected server error occurred. Please try again later.", request.getRequestURI(), List.of());
    }
